...
class Window(Frame) :
    ...
    def init_window(self) :
        ...
        quitbutton = Button(self, text="Quit", bg='green', fg='black', command=self.clint_exit)
        quitbutton.place(x=250, y=150)  # adjust the number of pixels of button
        
        NewPassBtn = Button(self, text="Add a new password",  bg='green', fg='black', command=self.StorePassword)

        PassDirBtn = Button(self, text="View password directory", command=self.SeePassword)
        
        EnterButton = Button(self,text="Enter", bg='green', fg='black',  command=lambda : Unlock(nameEntered.get()))
        ...
        RetryBtn = Button(self, text= "OK ill retry!!",bg='red', fg='black', command=lambda : retryfunc(self))
        ...
        def Unlock(Passw):
            if str(Passw) == "Zoraiz":
                ...
                trash = PhotoImage(file='trashjpg.png')

                canvas.create_image(20, 20, anchor=NW, image= trash)
                mainloop()
